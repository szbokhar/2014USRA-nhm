% MATLAB script to interactively segment insect specimens given a template
%
...
function [rankings, placed, placed_cut] = random_template_interact(fname, pcount, off_x, off_y, scales)
    ...
    for n = [1:150]
        ...
        for pwr = scales
            ...
            [tScores, maxtscore] = useTemplate(pts, base, stack, swidth, sheight, off_x, off_y, int32(cx/off_x), int32(cy/off_y));
        ...
        [by,bx,bs] = ind2sub(size(allScores), find(allScores == max(allScores(:))))
        plot(bx*off_x, by*off_y, 'oy');
        sca = scales(bs);
        rectangle('position', [bx*off_x-(sca*box_width/2), by*off_y-(sca*box_height/2), sca*box_width, sca*box_height], 'EdgeColor', 'g');
    end
end


% Pass template over image
function [tScores maxtscore] = useTemplate(pts, base, stack, box_width, box_height, off_x, off_y, px, py)
    ...
    for j=int32(py-box_height/(4*off_y)):int32(py+box_height/(4*off_y))
        for i=int32(px-box_width/(4*off_x)):int32(px+box_height/(4*off_x))
